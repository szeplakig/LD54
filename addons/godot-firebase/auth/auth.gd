...
var _oauth_login_request_body: Dictionary = {
	"postBody": "", "requestUri": "", "returnIdpCredential": false, "returnSecureToken": true
}
...
var _custom_token_body: Dictionary = {"token": "", "returnSecureToken": true}
...
func _check_emulating() -> void:
	...
	if not Firebase.emulating:
		_base_url = "https://identitytoolkit.googleapis.com/{version}/".format(
		...
		if port == "":
			(
				Firebase
				. _printerr(
					"You are in 'emulated' mode, but the port for Authentication has not been configured."
				)
			)
		else:
			_base_url = "http://localhost:{port}/identitytoolkit.googleapis.com/{version}/".format(
				{version = _API_VERSION, port = port}
			)
			_refresh_request_base_url = "http://localhost:{port}/securetoken.googleapis.com".format(
				{port = port}
			)
...
func signup_with_email_and_password(email: String, password: String) -> void:
	if _is_ready():
		...
		var err = request(
			_base_url + _signup_request_url,
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify(_login_request_body)
		)
...
func login_anonymous() -> void:
	if _is_ready():
		...
		var err = request(
			...
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify(_anonymous_login_request_body)
		)
		if err != OK:
			is_busy = false
...
func login_with_custom_token(token: String) -> void:
	if _is_ready():
		...
		var err = request(
			_base_url + _signin_custom_token_url,
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify(_custom_token_body)
		)
		if err != OK:
			is_busy = false
	...
	if _token:
		...
		if is_successful and _is_ready():
			...
			_oauth_login_request_body.postBody = (
				"access_token=" + token + "&providerId=" + provider.provider_id
			)
...
func exchange_token(
	...
	if _is_ready():
		...
		var err = request(
			request_url, _headers, HTTPClient.METHOD_POST, JSON.stringify(exchange_token_body)
		)
		if err != OK:
			is_busy = false
...
func manual_token_refresh(auth_data):
	...
	var err = request(
		_refresh_request_base_url + _refresh_request_url,
		_headers,
		HTTPClient.METHOD_POST,
		JSON.stringify(_refresh_request_body)
	)
	if err != OK:
		is_busy = false
...
func _on_FirebaseAuth_request_completed(
	...
	if response_code == 0:
		...
		res = {"error": {"code": "Connection error", "message": "Error connecting to auth service"}}
	else:
		if json == null:
...
func save_auth(auth: Dictionary) -> bool:
	var encrypted_file = FileAccess.open_encrypted_with_pass(
		"user://user.auth", FileAccess.WRITE, _config.apiKey
	)
...
func load_auth() -> bool:
	var encrypted_file = FileAccess.open_encrypted_with_pass(
		"user://user.auth", FileAccess.READ, _config.apiKey
	)
	var err = encrypted_file == null
	if err:
		Firebase._printerr(
			"Error Opening Firebase Auth File. Error Code: " + str(FileAccess.get_open_error())
		)
...
func change_user_email(email: String) -> void:
	if _is_ready():
		...
		var err = request(
			_base_url + _update_account_request_url,
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify(_change_email_body)
		)
		if err != OK:
			is_busy = false
...
func change_user_password(password: String) -> void:
	if _is_ready():
		...
		var err = request(
			...
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify(_change_password_body)
		)
		if err != OK:
			is_busy = false
...
func update_account(
	...
	displayName: String,
	photoUrl: String,
	deleteAttribute: PackedStringArray,
	...
	if _is_ready():
		...
		var err = request(
			...
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify(_update_profile_body)
		)
		if err != OK:
			is_busy = false
...
func send_account_verification_email() -> void:
	if _is_ready():
		...
		var err = request(
			_base_url + _oobcode_request_url,
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify(_account_verification_body)
		)
		if err != OK:
			is_busy = false
...
func send_password_reset_email(email: String) -> void:
	if _is_ready():
		...
		var err = request(
			...
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify(_password_reset_body)
		)
		if err != OK:
			is_busy = false
...
func get_user_data() -> void:
	if _is_ready():
		...
		var err = request(
			_base_url + _userdata_request_url,
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify({"idToken": auth.idtoken})
		)
		if err != OK:
			is_busy = false
...
func delete_user_account() -> void:
	if _is_ready():
		is_busy = true
		var err = request(
			_base_url + _delete_account_request_url,
			_headers,
			HTTPClient.METHOD_POST,
			JSON.stringify({"idToken": auth.idtoken})
		)
...
func get_token_from_url(provider: AuthProvider):
	var token_type: String = (
		provider.params.response_type if provider.params.response_type == "code" else "access_token"
	)
...
func get_FacebookProvider() -> FacebookProvider:
	return FacebookProvider.new(
		_config.auth_providers.facebook_id, _config.auth_providers.facebook_secret
	)


func get_GitHubProvider() -> GitHubProvider:
	return GitHubProvider.new(
		_config.auth_providers.github_id, _config.auth_providers.github_secret
	)


func get_TwitterProvider() -> TwitterProvider:
	return TwitterProvider.new(
		_config.auth_providers.twitter_id, _config.auth_providers.twitter_secret
	)
