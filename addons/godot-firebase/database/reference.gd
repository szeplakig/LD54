...
func push(data: Dictionary) -> void:
	...
	if _pusher.get_http_client_status() == HTTPClient.STATUS_DISCONNECTED:
		_pusher.request(
			_get_list_url() + _db_path + _get_remaining_path(),
			_headers,
...
func delete(reference: String) -> void:
	if _pusher.get_http_client_status() == HTTPClient.STATUS_DISCONNECTED:
		_pusher.request(
			_get_list_url() + _db_path + _separator + reference + _get_remaining_path(),
			_headers,
...
func _get_filter():
	...
	if _cached_filter != "":
		...
		if _filter_query.has(ORDER_BY):
			_cached_filter += (
				ORDER_BY + _equal_tag + _escaped_quote + _filter_query[ORDER_BY] + _escaped_quote
			)
			_filter_query.erase(ORDER_BY)
		else:
			_cached_filter += (
				ORDER_BY + _equal_tag + _escaped_quote + _key_filter_tag + _escaped_quote
			)  # Presumptuous, but to get it to work at all...
