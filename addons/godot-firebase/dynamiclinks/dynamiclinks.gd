	...
	if not Firebase.emulating:
		...
		if port == "":
			(
				Firebase
				. _printerr(
					"You are in 'emulated' mode, but the port for Dynamic Links has not been configured."
				)
			)
		else:
...
func generate_dynamic_link(
	long_link: String, APN: String, IBI: String, is_unguessable: bool
) -> void:
	if not _config.domainUriPrefix or _config.domainUriPrefix == "":
		generate_dynamic_link_error.emit(
			"Error: Missing domainUriPrefix in config file. Parameter is required."
		)
	...
	_request_list_node.request(
		_base_url, _headers, HTTPClient.METHOD_POST, JSON.stringify(_link_request_body)
	)
