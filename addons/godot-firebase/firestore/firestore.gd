...
enum Requests { NONE = -1, LIST, QUERY }  ## Firestore is not processing any request.  ## Firestore is processing a [code]list()[/code] request checked a collection.  ## Firestore is processing a [code]query()[/code] request checked a collection.
...
## @return FirestoreTask
func list(
	path: String = "", page_size: int = 0, page_token: String = "", order_by: String = ""
) -> FirestoreTask:
	var firestore_task: FirestoreTask = FirestoreTask.new()
...
func _check_emulating() -> void:
	...
	if not Firebase.emulating:
		...
		if port == "":
			Firebase._printerr(
				"You are in 'emulated' mode, but the port for Firestore has not been configured."
			)
		else:
			_base_url = "http://localhost:{port}/{version}/".format(
