...
enum DIRECTION { DIRECTION_UNSPECIFIED, ASCENDING, DESCENDING }
...
func where(field: String, operator: int, value = null, chain: int = -1):
	if operator in [OPERATOR.IS_NAN, OPERATOR.IS_NULL, OPERATOR.IS_NOT_NAN, OPERATOR.IS_NOT_NULL]:
		if (
			(chain in [OPERATOR.AND, OPERATOR.OR])
			or (query.has("where") and query.where.has("compositeFilter"))
		):
			var filters: Array = []
...
static func _cursor_object(value, before: bool) -> Cursor:
	...
	var cursor: Cursor = Cursor.new(
		parse.arrayValue.values if parse.has("arrayValue") else [parse], before
	)
	return cursor
...
func create_composite_filter(operator: int, filters: Array) -> Dictionary:
	return {compositeFilter = {op = OPERATOR.keys()[operator], filters = filters}}
