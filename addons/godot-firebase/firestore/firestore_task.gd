...
enum Task { TASK_GET, TASK_POST, TASK_PATCH, TASK_DELETE, TASK_QUERY, TASK_LIST }  ## A GET Request Task, processing a get() request  ## A POST Request Task, processing add() request  ## A PATCH Request Task, processing a update() request  ## A DELETE Request Task, processing a delete() request  ## A POST Request Task, processing a query() request  ## A POST Request Task, processing a list() request
...
func _on_request_completed(
	...
	var failed: bool = (
		bod is Dictionary and bod.has("error") and response_code != HTTPClient.RESPONSE_OK
	)
	from_cache = offline
...
func _handle_cache(
	offline: bool, data, encrypt_key: String, cache_path: String, body
) -> Dictionary:
