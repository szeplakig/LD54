...
## @return FunctionTask
func execute(
	function: String, method: int, params: Dictionary = {}, body: Dictionary = {}
) -> FunctionTask:
...
func _check_emulating() -> void:
	...
	if not Firebase.emulating:
		_base_url = "https://{zone}-{projectId}.cloudfunctions.net/".format(
			{zone = _config.functionsGeoZone, projectId = _config.projectId}
		)
	else:
		...
		if port == "":
			(
				Firebase
				. _printerr(
					"You are in 'emulated' mode, but the port for Cloud Functions has not been configured."
				)
			)
		else:
			_base_url = "http://localhost:{port}/{projectId}/{zone}/".format(
				{port = port, zone = _config.functionsGeoZone, projectId = _config.projectId}
			)


func _pooled_request(task: FunctionTask) -> void:
	if _offline:
		task._on_request_completed(
			HTTPRequest.RESULT_CANT_CONNECT, 404, PackedStringArray(), PackedByteArray()
		)
		return
...
func _on_pooled_request_completed(
	result: int,
	response_code: int,
...
func _check_auth_error(code: int, message: String) -> void:
	...
	match code:
		400:
			err = "Please, enable Anonymous Sign-in method or Authenticate the Client before issuing a request (best option)"
