...
func _check_emulating() -> void:
	...
	if not Firebase.emulating:
		...
		if port == "":
			Firebase._printerr(
				"You are in 'emulated' mode, but the port for Storage has not been configured."
			)
		else:
...
func _upload(
	data: PackedByteArray, headers: PackedStringArray, ref: StorageReference, meta_only: bool
) -> StorageTask:
...
func _download(ref: StorageReference, meta_only: bool, url_only: bool) -> StorageTask:
	...
	info_task.action = (
		StorageTask.Task.TASK_DOWNLOAD_URL if url_only else StorageTask.Task.TASK_DOWNLOAD_META
	)
...
func _process_request(task: StorageTask) -> void:
	...
	if not (
		_http_client.get_status() in [HTTPClient.STATUS_CONNECTED, HTTPClient.STATUS_DISCONNECTED]
	):
		_http_client.close()
	...
	match task.action:
		...
		StorageTask.Task.TASK_DOWNLOAD_URL:
			...
			if json != null and json.has("downloadTokens"):
				task.data = (
					_base_url + _get_file_url(task.ref) + "?alt=media&token=" + json.downloadTokens
				)
			else:
				task.data = ""
